...
+ (void)drawToLayer:(CALayer *)layer area:(CGRect)area models:(NSArray<CCKLineModel *> *)models minMax:(CCMinMaxModel *)minMaxModel {
    ...
    CCWRPainter *sublayer = [[CCWRPainter alloc] init];
    sublayer.frame = area;
    ...
    [models enumerateObjectsUsingBlock:^(CCKLineModel * _Nonnull m, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        CGPoint point1 = CGPointMake(x + w / 2, (m.WR.WR1.floatValue - minMaxModel.min) * unitValue);
        CGPoint point2 = CGPointMake(x + w / 2, (m.WR.WR2.floatValue - minMaxModel.min) * unitValue);
        if (idx == 0) {
